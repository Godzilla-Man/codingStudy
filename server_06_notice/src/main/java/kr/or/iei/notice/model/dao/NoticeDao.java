...
public class NoticeDao {
	...
	public int insertComment(Connection conn, NoticeComment comment) {
		...
		String query = "insert into tbl_notice_comment values (seq_notice_comment.nextval, ?, ?, default, ?)";
		
		try {
			pstmt = conn.prepareStatement(query);
			pstmt.setString(1, comment.getCommentWriter());
			pstmt.setString(2, comment.getCommentContent());
			pstmt.setString(3, comment.getNoticeRef());
			result = pstmt.executeUpdate();
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public ArrayList<NoticeComment> selectNoticeComment(Connection conn, String noticeNo) {
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		
		ArrayList<NoticeComment> commentList = new ArrayList<NoticeComment>();
		
		String query = "select comment_no, comment_content, comment_date, notice_ref,(select member_id from tbl_member where member_no = comment_writer) as comment_writer from tbl_notice_comment where notice_ref = ?";
		...
		try {
			...
			while(rset.next()) {
				...
				comment.setCommentWriter(rset.getString("comment_writer"));
				comment.setCommentDate(rset.getString("comment_date"));
				comment.setNoticeRef(rset.getString("notice_ref"));
