...
public class UpdateServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * 문제점) 회원 정보 수정 후, 마이페이지로 이동했을 때 수정 이전의 정보가 화면에 보여짐.
		 *  - 원인은, myPage.jsp에서 세션에 저장된 회원 정보를 출력하는데 우리가 업데이트 해준것은 DB에 있는 정보임! 세션은 업데이트 되지 않았음.
		 * 
		 * 해결 방법1)
		 * 	- 수정이 잘 되었을 때, 세션에서 회원의 ID와, PW를 추출하여, service에 memberLogin()를 재사용한다.
		 *  - 반환된 Member 객체를 세션에 등록한다. 동일한 loginMember라는 key로 
		 *  - 단, 이 방법은 서블릿에서 service에 메소드를 두개 호출해야 함(updateMember, memberLogin) -> 디자인 패턴에 적절하지 않음.
		 *  
		 * 해결 방법2)
		 *  - 수정이 잘 되었을 때, 세션에 저장된 회원 정보를, 사용자가 입력한 정보로 업데이트한다.
		 * 
		 * 해결 방법3)
		 *  - 수정이 잘 되었을 때, 재 로그인 할 수 있도록 처리.
		 *  - 세션을 파기시키고, 재 로그인을 하면 DB에 있는 정보를 조회하여 세션에 등록하므로, 변경해주는 작업은 하지 않아도 됨.
		 *  	- 세션 파기 코드 == session.invalidate();
		 *  	- 이 코드는 LogoutServlet에 작성되어 있음. 재사용 가능!
		 *  	  response.sendRedirect("/member/logout");
		 *        단, 재사용 시, 브라우저 상단 URL이 /member/logout으로 변경됨.
		 *        
		 */
		...
		if(result > 0) {			
			...
			if(session != null) {
				Member loginMember = (Member) session.getAttribute("loginMember"); //로그인 했을 떄, 등록한 회원 정보
