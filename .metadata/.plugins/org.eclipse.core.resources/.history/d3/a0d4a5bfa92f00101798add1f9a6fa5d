...
public class NoticeService {
	...
	public Notice selectOneNotice(String noticeNo, boolean updChk) {
		...
		if(notice != null) {
			...
			/*
			 * updChk == true : 상세보기 이동 시 호출 => 이때만 조회 수 업데이트
			...
			if(result > 0 || !updChk) {
				...
				notice.setCommentList(commentList);
	...
	public ArrayList<NoticeFile> updateNotice(Notice notice, ArrayList<NoticeFile> addFileList,
		...
		ArrayList<NoticeFile> preFileList = null;
		...
		if(result > 0) {
			...
			if(delFileNoList != null) { //삭제할 파일이 있을 때
				String delFileNoStr = String.join("|", delFileNoList); //"2|5|3|1|4"
				
				//preFileList에서 삭제할 파일 정보만 남기고 remove
				for(int i=preFileList.size()-1; i >= 0; i--) {
					String preFileNo = String.valueOf(preFileList.get(i).getFileNo()); //기존 파일 번호 : 5, 4, 3, 2, 1
			...
			for(int i=0; i<addFileList.size(); i++) {
				NoticeFile insFile = addFileList.get(i);
