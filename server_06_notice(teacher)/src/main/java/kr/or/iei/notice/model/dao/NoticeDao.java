...
public class NoticeDao {
	...
	public ArrayList<Notice> selectNoticeList(Connection conn, int start, int end) {
		...
		String query = "SELECT NOTICE_NO, NOTICE_TITLE, (SELECT MEMBER_ID FROM TBL_MEMBER WHERE MEMBER_NO = NOTICE_WRITER) AS NOTICE_WRITER, NOTICE_DATE, READ_COUNT FROM ( SELECT ROWNUM RNUM, A.* FROM ( SELECT * FROM TBL_NOTICE A ORDER BY NOTICE_DATE DESC ) A ) WHERE RNUM >= ? AND RNUM <= ?";
