...
<body>
	...
	<script>
//브라우저에 html, css가 모드 로드되고 난 이후에 자동으로 실행되는 함수
	...
	const checkObj = {
		...
		"memberPhone": false,
		"memberEmail":false,
		"idDuplChk" : false //아이디 중복체크 여부(true면 중복X, false면 중복O 또는 중복체크 진행하지 않았다)
	};
	...
	$(idDuplChkBtn).on('click', function(){
		...
		*/
		...
		/* 위 방식대로 요청 및 서블릿에서 기존 서블릿과 동일하게 4. 결과 처리 진행하면
		   join.jsp를 브라우저에 다시 그림. 즉, 입력값들이 초기화되어진다.
		   
		   - 비동기 통신 기술 중, ajax라는 기술을 이용하면 페이지 전환 없이 데이터만
		     서버로부터 응답 받아, 동적으로 페이지의 일부분을 업데이트 해줄 수 있음.
